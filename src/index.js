    ...
    build(callback) {
        ...
        build(webpackConfig, () => {
            ...
            ssrWebpackConfig.plugins = ssrWebpackConfig.plugins
                                                       .filter(plugin =>
                                                           ignorePluginsType.findIndex(Type => plugin instanceof Type)
                                                           < 0
                                                       )
            ...
            buildMethod(ssrWebpackConfig, () => {
                ...
                function logPath(path) {
                    count++
                    console.log(chalk.green(' `' + nps.relative(process.cwd(), path)
                            ...
                            return new Promise((resolve, reject) => {
                                method('/' + path.replace(/^\/+/, ''), async (content, renderProps) => {
                                    if (content == null) {
                                        ...
                                        let actualTemplateData = typeof templateData === 'function'
                                            ? templateData(inputArg, 'prod')
                                            : templateData
                                                        ...
                                                        let newSites = obj.hrefList
                                                                          .map(href => {
                                                                              if (href.startsWith(publicPath)) {
                                                                                  href =
                                                                                      href.substring(publicPath.length)
