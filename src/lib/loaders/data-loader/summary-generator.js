    ...
    ${JSON.stringify(name)}: function () {
        return new Promise(function (resolve) {
            require.ensure([], function (require) {
                resolve(require(${JSON.stringify(
                  '!!' + toUriPath(mdLoaderPath) + '!' + toUriPath(require)
                )}));
            }, ${JSON.stringify(name)})
        })
    },`
...
async function generatePickedMeta(
  filesMap,
  {
    picker,
    ...
    function getMarkdownData(md = content) {
      return new Promise((resolve, reject) => {
        generate(
          md,
          infoData,
          function(err, meta, data) {
            if (err) {
              reject(err)
            } else {
              resolve(
                chain(htmlTransformers, data, {
                  meta: { ...meta },
                  path,
                  filesMap: { ...filesMap }
                })
...
function pluginsStr(plugins = []) {
  return plugins
    .map(
      ({ path, opt }) =>
        `(require('${toUriPath(path)}').default || require('${toUriPath(
          path
        )}'))(${JSON.stringify(opt)})`
    )
    .join(',')
...
async function summaryGenerate(
  ...
  return Promise.resolve(
    '{' +
      '\n  lazyload: {' +
      generateLazyLoad(filesMap, lazyload) +
      '  },' +
      '\n  meta: ' +
